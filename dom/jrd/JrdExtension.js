...
jrdIDOMExtension.prototype = {
  ...
  init: function(aWindow) {
    ...
    this.initDOMRequestHelper(aWindow, ['JrdSrv:Common:Return',
      ...
      'JrdSrv:ReadNvitemRoot:Return', //czb@tcl.com add for read Nvitem root check.
      'JrdSrv:ReadNvitem:Return', //jrd_tanya add for read Nvitem.
      'JrdSrv:WriteNvitem:Return', //jrd_tanya add for read Nvitem.
      'JrdSrv:ReadNvitemEx:Return', //liukun@tcl.com add for read Nvitem.
      'JrdSrv:WriteNvitemEx:Return' //liukun@tcl.com add for write Nvitem.
      ]);
    ...
    switch (aProcessType) {
      ...
      case 'qxdm':
          if(aStoreToDevice === 'INTERNAL_SDCARD'){
              run_str = '/system/bin/diag_mdlog -e -f /storage/sdcard0/jrdlog/qxdm/diag.cfg';
          }else{
              run_str = '/system/bin/diag_mdlog -e -f /storage/sdcard1/jrdlog/qxdm/diag.cfg';
          }
        break;
  ...
  _getDestDicPath: function(aStoreToDevice, aProcessType) {
    let directoryPath;
//fix bug #641051 change phone to internalsdcard && change sdcard to externalsdcard
    if (aStoreToDevice === 'INTERNAL_SDCARD') {
      directoryPath = '/storage/sdcard0/jrdlog/' + aProcessType + '/';
    }
    else { //EXTERNAL_SDCARD
  ...
  _getDestWholePath: function(aStoreToDevice, aProcessType) {
    ...
    destStr += aProcessType + '_' + this._currentDateTime + '.log';//congqb@tcl.com Add extention for the log file for bug #631080
...
//fix bug #641051 change phone to internalsdcard && change sdcard to externalsdcard
        //12: the length of '/mnt/sdcard1'
        if (obj[typeStr].location.substr(0, 16) === '/storage/sdcard1') {
          data[typeStr].location = 'External_Sdcard';
        }
        else {
          data[typeStr].location = 'Internal_Sdcard';
        }
      }
      else {
        ...
        if (requestId === aMsg.requestID) {
          ...
          if (aProcessState === 'process-finished') {
            if('qxdm' === type && 'JrdSrv:CopyFile:Return' === aMessage.name) {
              ...
              let aFilePathArray = [];
    //fix bug #641051 change phone to internalsdcard && change sdcard to externalsdcard
              aFilePathArray[0] = this._getDestDicPath('INTERNAL_SDCARD', type) + type + '*';
              aFilePathArray[1] = this._getDestDicPath('EXTERNAL_SDCARD', type) + type + '*';
  ...
  _copyTombstones: function(requestId) {
    ...
    let fileName = aParam + '_' + this._currentDateTime + '.log'; //congqb@tcl.com Add extention for the log file for bug #631080 
  ...
  _copyDiagCfg: function(requestId) {
    ...
    let sourcePath = '/system/vendor/diag.cfg';              //fix bug #662074 QXDM looging, unable to collect logs
    let directoryPath = this._getDestDicPath(this._requestInfo[requestId].storeToDevice,
       this._requestInfo[requestId].type);
    let fileName = null;
...
/*----jrd_tyin add for UniversalCommand start----*/
 ...
 // fix bug# 662328 congqb@tcl.com 
  startUniversalCommandPre: function(command, isUseShell ,type ){
    debug('startUniversalCommandPre '+command+' '+isUseShell+' '+type);  
...
/*--- tcl_chenguoqiang for PR 614234 ---*/
  setChargingEnabled: function(config) {
    debug('SetChargingEnabled');
    cpmm.sendAsyncMessage('JrdSrv:SetChargingEnabled', {config: config});
  },
...
/*----czb add for get root check  data start----*/
  readNvitemRoot: function(item) {
   debug('JrdExtension readNvitemRoot');
   let request = this.createRequest();
   cpmm.sendAsyncMessage('JrdSrv:ReadNvitemRoot', {item: item, requestID: this.getRequestId(request)});
   return request;
  },
/*----czb@tcl.com add for get root check  data end----*/

/*----liukun@tcl.com add for get NV data start----*/
  readNvitemEx: function(item) {
   debug('JrdExtension readNvitemEx');
   let request = this.createRequest();
   cpmm.sendAsyncMessage('JrdSrv:ReadNvitemEx', {item: item, requestID: this.getRequestId(request)});
   return request;
  },
  writeNvitemEx: function(item, length, value) {
   debug('JrdExtension writeNvitemEx');
   let request = this.createRequest();
   cpmm.sendAsyncMessage('JrdSrv:WriteNvitemEx', {item: item, length: length, value: value, requestID: this.getRequestId(request)});
   return request;
  },
/*----liukun@tcl.com end for get NV data end----*/
...
//fix bug #641051 change phone to internalsdcard && change sdcard to externalsdcard
      aFilePathArray[aFilePathArray.length] =
        this._getDestDicPath('INTERNAL_SDCARD', aParamAarry[i]) + aParamAarry[i] + '*';
      aFilePathArray[aFilePathArray.length] =
        this._getDestDicPath('EXTERNAL_SDCARD', aParamAarry[i]) + aParamAarry[i] + '*';
    }
...
//czb@tcl.com add for root check start.
      case 'JrdSrv:ReadNvitemRoot:Return':
        debug('czb:'+ msg.requestID);
        request = this.takeRequest(msg.requestID);
        if (request) {
          if (msg.data.result === 'OK') {
            debug('Success: ' + JSON.stringify(msg));
...
//liukun@tcl.com add for read nvram return start.
      case 'JrdSrv:ReadNvitemEx:Return':
        debug('jrdextension.js JrdSrv:ReadNvitemEx:Return:'+ msg.requestID);
        request = this.takeRequest(msg.requestID);
        if (request) {
          if (msg.data.result === 'OK') {
            debug('Success: ' + JSON.stringify(msg));
      ...
      case 'JrdSrv:WriteNvitemEx:Return':
        debug('jrdextension.js case JrdSrv:WriteNvitemEx:Return' + msg.requestID);
        request = this.takeRequest(msg.requestID);
        Services.DOMRequest.fireSuccess(request, exposeWrite(msg.data.data));

  	break;
//liukun@tcl.com add for read nvram return end.
      default:
  ...
  getSysCode: function(code) {
    ...
    if ('*#3228#' === code) {
      ...
      a = '1005';
    }  else if ('*#*#212018#*#*' === code) { //TCL_zmm Add CR 612961
      a = '1006';
    } else if ('*#06#' === code) {
      a = '2001';
    } else if ('*#837837#' === code) {
    ...
    } else if ('*#0601#' === code) {//gaolu add for testbox
      a = '3001';
