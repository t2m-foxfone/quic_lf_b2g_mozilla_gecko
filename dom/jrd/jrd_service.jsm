...
this.JrdService = {
  init: function() {
    ...
    this._messages = ['JrdSrv:Common',
      ...
      'JrdSrv:ReadNvitemRoot', //czb@tcl.com for ReadNvitemRoot
      'JrdSrv:ReadNvitem', //jrd_tanya for ReadNvitem
      'JrdSrv:WriteNvitem', //jrd_tanya for WriteNvitem
      'JrdSrv:ReadNvitemEx', // liukun@tcl.com for ReadNvitemEx
      'JrdSrv:WriteNvitemEx', // liukun@tcl.com for WriteNvitemEx
      'JrdSrv:SetChargingEnabled', // tcl_chenguoqiang for PR 614234
 ...
 _readSysFile: function(filePath, callback) {
  ...
  //jrd_tanya add for readNvitem end.
   ...
   //czb@tcl.com add for readNvitemRoot start.
   _readNvitemRoot: function(item,callback){
    debug("jrd_service.jsm _readNvitem: enter");
    let obj = {};
    let jrdNvAccess = Components.classes['@jrdcom.com/JrdNvAccess;1'].createInstance(Components.interfaces.nsIJrdNvAccess);
    try {
      obj.data = jrdNvAccess.readNvitemRoot(item);
      obj.result = 'OK';
      debug("jrd_service.jsm _readNvitemRoot: size: " + obj.data.length +  ", value: " + obj.data.toString());
    }
    catch(e) {
      obj.result = 'KO';
      debug("read nvram fail: " + e.result);
  ...
  //czb@tcl.com add for readNvitemRoot end.

  //liukun@tcl.com add for readNvitemEx start.
   _readNvitemEx: function(item,callback){
    debug("jrd_service.jsm _readNvitemEx: enter");
    let obj = {};
    let jrdNvAccess = Components.classes['@jrdcom.com/JrdNvAccess;1'].createInstance(Components.interfaces.nsIJrdNvAccess);
    try {
      obj.data = jrdNvAccess.readNvitemEx(item);
      obj.result = 'OK';
      debug("jrd_service.jsm _readNvitemEx: size: " + obj.data.length +  ", value: " + obj.data.toString());
    }
    catch(e) {
      obj.result = 'KO';
  ...
  //liukun@tcl.com end for readNvitemEx end.
  ...
  //liukun@tcl.com add for writeNvitemEx start.
  ...
  //liukun@tcl.com end for writeNvitemEx end.
  ...
  _setchargerled: function(config) {
    ...
    try {
      let file = Components.classes['@mozilla.org/file/local;1'].createInstance(Components.interfaces.nsILocalFile);
      file.initWithPath("/sys/class/leds/red/brightness");
  ...
  // tcl_chenguoqiang add for PR 614234
    ...
    switch (aMessage.name) {
      ...
      case 'JrdSrv:AudioLoop':
        if(msg.operation === 'start') {
          ...
          if(msg.param === 'mic') {
            args = ['-tc', '19', '-c', '/etc/ftm_test_config', '-d', '2', '-v', '80'];
          }
          else if(msg.param === 'sub-mic') {
            args = ['-tc', '28', '-c', '/etc/ftm_test_config', '-d', '2', '-v', '67'];
          }
          else if(msg.param === 'headset-left') {
            args = ['-tc', '6', '-c', '/etc/ftm_test_config', '-d', '2', '-v',
                 '60', '-file', '/system/vendor/112.wav'];
          }
          else if(msg.param === 'headset-right') {
            args = ['-tc', '7', '-c', '/etc/ftm_test_config', '-d', '2', '-v',
                 '60', '-file', '/system/vendor/112.wav'];
          }
          else if(msg.param === 'headset-mic') {
            args = ['-tc', '22', '-c', '/etc/ftm_test_config', '-d', '2', '-v', '88'];
          }
          else {
      ...
      //czb@tcl.com add for ReadNvitemRoot end
      case 'JrdSrv:ReadNvitemRoot':
        this._readNvitemRoot(
          msg.item,
          function(data){
          mm.sendAsyncMessage('JrdSrv:ReadNvitemRoot:Return', {data: data, requestID: msg.requestID});
        });
      break;
      //czb@tcl.com add for ReadNvitemRoot end

      //liukun@tcl.com add for ReadNvitemEx start
      case 'JrdSrv:ReadNvitemEx':
        this._readNvitemEx(
          msg.item,
          function(data){
          mm.sendAsyncMessage('JrdSrv:ReadNvitemEx:Return', {data: data, requestID: msg.requestID});
        });
      break;
      //liukun@tcl.com end for ReadNvitemEx end
	  ...
	  msg.value,
          function(){
          debug("jrdnvram write finished.");
          mm.sendAsyncMessage('JrdSrv:WriteNvitemEx:Return', {data:'', requestID: msg.requestID});
        });
      break;
      //liukun@tcl.com add for WriteNvitemEx end
      ...
      case 'JrdSrv:SetChargingEnabled':
